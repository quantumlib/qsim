...
def _test_linux_basic_impl(ctx):
    ...
    cmd_output = (
        ...
        "rdtscp lm constant_tsc rep_good nopl xtopology cpuid " +
        "tsc_known_freq pni pclmulqdq vmx ssse3 cx16 pcid movbe popcnt " +
        "tsc_deadline_timer aes xsave avx f16c rdrand hypervisor lahf_lm " +
        "abm 3dnowprefetch cpuid_fault pti ssbd ibrs ibpb stibp tpr_shadow " +
        "vnmi flexpriority ept vpid ept_ad fsgsbase tsc_adjust erms " +
        "invpcid rdseed adx smap clflushopt xsaveopt xsavec xgetbv1 xsaves " +
    ...
    feature_summary = get_feature_booleans(features)
    asserts.true(env, feature_summary.has_avx)
    asserts.true(env, feature_summary.has_sse)
    asserts.true(env, "AVX" in feature_summary.cpu_features_str)
    asserts.true(env, "SSE2" in feature_summary.cpu_features_str)
...
def _test_linux_avx2_impl(ctx):
    ...
    cmd_output = (
        ...
        "cmov pat pse36 clflush mmx fxsr sse sse2 ss ht syscall nx pdpe1gb " +
        "rdtscp lm constant_tsc rep_good nopl xtopology cpuid " +
        "tsc_known_freq pni pclmulqdq vmx ssse3 fma cx16 pcid sse4_1 " +
        "sse4_2 x2apic movbe popcnt tsc_deadline_timer aes xsave avx f16c " +
        "rdrand hypervisor lahf_lm abm 3dnowprefetch cpuid_fault pti ssbd " +
        ...
        "smap clflushopt xsaveopt xsavec xgetbv1 xsaves arat umip " +
        "arch_capabilities"
    )
...
def _test_linux_avx512_impl(ctx):
    ...
    cmd_output = (
        "flags : ... avx512f avx512dq avx512cd avx512bw avx512vl avx2 " +
        "bmi2 ..."
    )
...
def _test_macos_impl(ctx):
    ...
    cmd_output = (
        ...
        "SSE2 SS HTT TM PBE SSE3 PCLMULQDQ DTES64 MON DSCPL VMX EST TM2 " +
        "SSSE3 FMA CX16 TPR PDCM SSE4.1 SSE4.2 x2APIC MOVBE POPCNT AES " +
        "PCID XSAVE OSXSAVE SEGLIM64 VMM TSCTMR AVX1.0 RDRAND F16C\n" +
        "machdep.cpu.leaf7_features: SMEP ERMS RDWRFSGS TSC_THREAD_OFFSET " +
        "BMI1 HLE AVX2 BMI2 INVPCID RTM RDSEED ADX SMAP CLFLUSHOPT IPT"
    )
