...
class StateSpaceAVX : public StateSpace<StateSpaceAVX<For>, For, float> {
 ...
 public:
  ...
  void SetStateUniform(State& state) const {
    ...
    auto f = [](unsigned n, unsigned m, uint64_t i, __m256& val0, __m256 valu,
                fp_type* p) {
      _mm256_store_ps(p + 16 * i, valu);
      _mm256_store_ps(p + 16 * i + 8, val0);
    };

    Base::for_.Run(MinSize(state.num_qubits()) / 16, f, val0, valu,
                   state.get());
  }
  ...
  static void SetAmpl(State& state, uint64_t i,
  ...
  bool Add(const State& src, State& dest) const {
    ...
    auto f = [](unsigned n, unsigned m, uint64_t i, const fp_type* p1,
                fp_type* p2) {
  ...
  std::complex<double> InnerProduct(const State& state1,
    ...
    auto f = [](unsigned n, unsigned m, uint64_t i, const fp_type* p1,
                const fp_type* p2) -> std::complex<double> {
      __m256 re1 = _mm256_load_ps(p1 + 16 * i);
      __m256 im1 = _mm256_load_ps(p1 + 16 * i + 8);
      __m256 re2 = _mm256_load_ps(p2 + 16 * i);
      __m256 im2 = _mm256_load_ps(p2 + 16 * i + 8);
    ...
    return Base::for_.RunReduce(MinSize(state1.num_qubits()) / 16, f, Op(),
  ...
  std::vector<uint64_t> Sample(const State& state, uint64_t num_samples,
                               unsigned seed) const {
  ...
  void Collapse(const MeasurementResult& mr, State& state) const {
    auto f1 = [](unsigned n, unsigned m, uint64_t i, uint64_t mask,
                 uint64_t bits, const fp_type* p) -> double {
    ...
    auto f2 = [](unsigned n, unsigned m, uint64_t i, uint64_t mask,
                 uint64_t bits, __m256 renorm, fp_type* p) {
      __m256i ml = detail::GetZeroMaskAVX(8 * i, mask, bits);
    ...
    Base::for_.Run(MinSize(state.num_qubits()) / 16, f2, mr.mask, mr.bits,
                   renorm, state.get());
  }
  ...
  std::vector<double> PartialNorms(const State& state) const {
    ...
    using Op = std::plus<double>;
    return Base::for_.RunReduceP(MinSize(state.num_qubits()) / 16, f, Op(),
                                 state.get());
  }

  uint64_t FindMeasuredBits(unsigned m, double r, uint64_t mask,
