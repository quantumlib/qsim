...
class AutoScaler():
    ...
    def getInstanceTemplateInfo(self):
        ...
        requestInstanceTemplate = \
            self.service.instanceTemplates().get(project=self.project,
        ...
        request = self.service.machineTypes().get(project=self.project, zone=self.zone,
    ...
    def scale(self):
        ...
        if queue > 0:
            ...
            if self.debug>0:
                print("Calucalting size of MIG: ⌈" + str(queue) + "/" + str(self.cores_per_node) + "⌉ = " + str(self.size))
        else:
            self.size = 0
        ...
        if self.compute_instance_limit > 0 and self.size > self.compute_instance_limit:
            ...
            print("MIG target size will be limited by " + str(self.compute_instance_limit))
